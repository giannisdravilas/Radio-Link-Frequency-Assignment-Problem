...
int main(void){
    ...
    strcpy(not_available, "ΜΗ");

    char* available = malloc(10*sizeof(char));
    strcpy(available, "ΔΙ");

    char* low_stock = malloc(10*sizeof(char));
    strcpy(low_stock, "ΜΙ");
    ...
    i = 0;
    // fgets(new_state[i], 100, stdin);
    // printf("new state %d is %s\n", i, new_state[i]);
    // while(strcmp(new_state[i], "0") != 0){
    //     i++;
    //     fgets(new_state[i], 100, stdin);
    //     printf("new state %d is %s\n", i, new_state[i]);
    // }
    ...
    for(MapNode node = map_first(map_new); node != MAP_EOF; node = map_next(map_new, node)){
        if(strcmp(map_node_value(map_new, node), available) == 0 || strcmp(map_node_value(map_new, node), low_stock) == 0){
        ...
        }else if(strcmp(map_node_value(map_new, node), not_available) == 0){
            MapNode found = map_find_node(map_site, map_node_key(map_new, node));
            if(found){
